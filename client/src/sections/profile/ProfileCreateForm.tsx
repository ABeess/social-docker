...
export const ProfileCreateForm = () => {
  ...
  const { mutate, isLoading } = useMutation((values: ProfileInput) => updateProfile(values), {
    onSuccess({ user, profile, code, message }) {
      enqueueSnackbar('Update profile successfully');
      if (code === 200) {
        queryClient.setQueryData<ProfileUserResponse>(['PROFILE_USER', { user_id: user.id }], {
          code,
          message,
          user: {
            ...user,
